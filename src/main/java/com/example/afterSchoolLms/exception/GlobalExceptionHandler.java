...
public class GlobalExceptionHandler {
	...
	public String handleRefundProcessException(
		...
		// Slack Webhook 호출
		// 환불 실패 정보(enrollmentId, 예외 메시지)를 Slack 채널로 전송
		slackNotifier.notifyRefundFailure(e.getEnrollmentId(), e.getMessage());

		// 사용자에게 보여줄 메시지를 flash attribute로 저장
		// redirect 후에도 메시지를 전달하기 위해 사용
